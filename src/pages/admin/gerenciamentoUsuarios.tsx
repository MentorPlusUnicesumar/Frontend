...
export function GerenciamentoUsuarios() {
  ...
  const mentores = data?.filter(
    (user) => user.typeUser === "Mentor" && user.status !== "Analisando"
  );
  const alunos = data?.filter(
    (user) => user.typeUser === "Aluno" && user.status !== "Analisando"
  );
          ...
          <Tabs w={"full"} onChange={() => setFilter({})}>
            ...
            {isLoading ? (
              ...
              <TabPanels>
                <TabPanel>
                  <PainelUsuarios
                    listaUsuarios={mentores || []}
                    filterFunction={(filter) => handleFilter(filter)}
                  />
                </TabPanel>
                <TabPanel>
                  <PainelUsuarios
                    listaUsuarios={alunos || []}
                    filterFunction={(filter) => handleFilter(filter)}
                  />
                </TabPanel>
                <TabPanel>
                  <PainelUsuarios
                    listaUsuarios={novosCadastros || []}
                    filterFunction={(filter) => handleFilter(filter)}
                  />
                </TabPanel>
              </TabPanels>
